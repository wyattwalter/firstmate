...
FM_CHECK_INTERVAL=300   # seconds between slow checks (PR merge and bot-finding polls or the X-mode poll shim)
FM_CHECK_TIMEOUT=30     # seconds allowed per slow check script
FM_BOT_REVIEWER_EXTRA=  # optional extended-regex of extra author logins fm-pr-bot-findings.sh treats as automated reviewers, for a bot that posts under a plain user account (Bot-typed and <name>[bot] authors are detected already)
